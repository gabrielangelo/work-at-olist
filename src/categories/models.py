...
class Category(models.Model):
    ...
    def add_node(cls, root, parent):
            ...
            if check_parents.exists():
                ...
                #return cls(title=parent.title, channel=parent.channel, lft=last_node.rgt + 1, rgt=last_node.rgt + 2)
                ...
                node_queryset.create(title=parent.title, channel=parent.channel, lft=root.lft + 1, rgt=root.lft + 2,
                                     parent=root)
                #return cls(title=parent.title, channel=parent.channel, lft=root.lft + 1, rgt=root.lft + 2)
    ...
    def make_json_tree(cls, parent):
        ...
        for node in nodes:
            if node.parent_id is None:
                path.append(node)
                dump_tree(t, path)
            elif node.parent_id == path[-1:][-1].id:
                path.append(node)
                dump_tree(node)
            elif node.lft == path[-1:][-1].rgt + 2:
